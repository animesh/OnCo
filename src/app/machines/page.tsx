...
export const metadata: Metadata = pageMeta({ title: "Machines", path: "/machines/", description: "The scanners, accelerators, robots and ablation devices hospitals use against cancer: what each machine does, what it is used for, its advantages and limits against the alternatives, who makes it, and which centres run the rare ones such as proton and carbon-ion therapy, Gamma Knife and MR-linacs." });
...
const FAMILIES: Array<{ name: string; icon: string; blurb: string; ids: string[] }> = [
  { name: "Imaging scanners", icon: "imaging", blurb: "How tumours are found, staged and measured.", ids: ["x-ray-radiography-fluoroscopy", "ct", "dual-energy-spectral-ct", "photon-counting-ct", "ultrasound", "ultrasound-elastography-ceus", "mri", "mri-field-strengths", "mammography", "pet", "pet-ct", "pet-mri", "total-body-pet-screening", "spect", "spect-ct", "intraoperative-mri-ct", "optical-imaging"] },
  { name: "Radiotherapy machines", icon: "radiation", blurb: "Photon and electron beams, from the everyday linac to dedicated radiosurgery units.", ids: ["c-arm-linac", "ring-gantry-linac", "tomotherapy", "mr-linac", "cyberknife", "gamma-knife", "zap-x", "biology-guided-radiotherapy", "radiosurgery-srs", "brachytherapy-afterloaders", "intraoperative-radiotherapy", "superficial-radiotherapy", "cobalt-60-teletherapy"] },
  { name: "Particle therapy", icon: "radiation", blurb: "Protons, carbon ions and neutrons: the machines that stop inside the tumour.", ids: ["proton-therapy", "proton-therapy-systems", "carbon-ion", "carbon-ion-synchrotrons", "bnct"] },
  { name: "Ablation and energy devices", icon: "devices", blurb: "Heat, cold, sound, electric pulses and fields delivered through a needle or from outside.", ids: ["thermal-ablation", "cryoablation-systems", "microwave-rf-ablation", "irreversible-electroporation", "hifu-histotripsy", "bbb-focused-ultrasound", "litt", "electrochemotherapy", "hyperthermia", "ttfields"] },
  { name: "Surgical machines", icon: "surgery", blurb: "Robots, navigation and cameras in the operating theatre.", ids: ["robotic-surgery", "surgical-robot-platforms", "robotic-bronchoscopy", "fluorescence-guided-surgery"] },
  { name: "Laboratory and production", icon: "diagnostics", blurb: "The instruments behind pathology, blood tests and tracer supply.", ids: ["whole-slide-scanners", "flow-cytometers", "spatial-biology-instruments", "medical-cyclotrons-synthesis-modules"] },
];

/** Machines rare enough that where they are matters; the location section lists every centre in the corpus that runs each. */
const EXOTIC = ["proton-therapy-systems", "carbon-ion-synchrotrons", "mr-linac", "gamma-knife", "cyberknife", "tomotherapy", "zap-x", "biology-guided-radiotherapy", "hifu-histotripsy", "ttfields", "bnct"];
