...
import { simpleR } from "@/data/simple/part-r";
import { simpleS } from "@/data/simple/part-s";
 ...
 */
const NEW_PARTS: Record<string, Record<string, string>> = { "part-o": simpleO, "part-p": simpleP, "part-q": simpleQ, "part-r": simpleR, "part-s": simpleS };
...
describe("simple layer", () => {
  ...
  it("part-r holds at least 1300 sentences", () => {
    expect(Object.keys(simpleR).length).toBeGreaterThanOrEqual(1300);
  });

  it("part-s holds at least 160 sentences", () => {
    expect(Object.keys(simpleS).length).toBeGreaterThanOrEqual(160);
  });
});
